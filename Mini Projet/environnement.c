...
int etat = 0;
bool M_juste_avant = false;

/* Initialise l'environnement envt :
   - lit le terrain dans le fichier ficenvironnementhier_terrain
...
resultat_deplacement avancer_envt(Environnement *envt) {
  observateur_MA(&M_juste_avant, 'A');
  observateur_GD(&etat, 'A');
...
{
  observateur_MA(&M_juste_avant, 'G');
  observateur_GD(&etat, 'G');
  tourner_a_gauche(&(envt->r));
}

/* Tourner le robot à droite */
void droite_envt(Environnement *envt) {
  observateur_MA(&M_juste_avant, 'D');
  observateur_GD(&etat, 'D');
...
int mesure_envt(Environnement *envt, int d) {
  observateur_MA(&M_juste_avant, 'M');
  observateur_GD(&etat, 'M');
